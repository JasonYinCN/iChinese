...
@SuppressLint("HandlerLeak")
 public class HorizontalScrollBarStrip extends HorizontalScrollView {
    /**
     * tag标题
     */
    ...
    private Integer mActiveTextSize = 20;
    /**
     * 默认参数
     */
    private Integer mDefaultColor = Color.BLACK;
    private Integer mDefaultTextSize = 16;

    public HorizontalScrollBarStrip(Context context) {
        this(context, null);
    }

    public HorizontalScrollBarStrip(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HorizontalScrollBarStrip(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void resetLayoutParams() {
        ...
        if (null == mTagLayoutParams) {
            mTagLayoutParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
        }
...
//        Toast.makeText(getContext(), mTitles.get(position), Toast.LENGTH_SHORT).show(); /** 当前控件索引 */
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == position) {
                ...
                tv.setTextSize(mActiveTextSize);
            } else {
        ...
        if (location[0] < 0) {
            ...
            if (mTagIndex < tabsContainer.getChildCount()) {
                motionLength = mDefaultTagWidth - (mScreenWidth - location[0]) + mDefaultTagWidth / 2;
            } else {
